...
public class GameController : MonoBehaviour
{
    ...
    private GameObject _activeTeam;
    ...
    void FocusOnCurrentWorm()
    {
        ...
        _activeTeam = GameObject.Find("Team_" + _currentTeam);
        int health = _activeTeam.transform.GetChild(_currentWorm).GetComponent<Worm>().GetHealth();
        while (health <= 0)
        {
            ++_currentWorm;
            //print(team.transform.GetChild(_currentWorm).name);
            health = _activeTeam.transform.GetChild(_currentWorm).GetComponent<Worm>().GetHealth();
        }
        _activeTeam.transform.GetChild(_currentWorm).GetComponent<Worm>().SetCurrent(true);
    ...
    private void SwitchWorm(InputAction.CallbackContext context)
    {
        ...
        if (context.ReadValue<float>() == 1.0f)
        {
            ...
            _activeTeam.transform.GetChild(_currentWorm).GetComponent<Worm>().SetCurrent(false);
            ++_currentWorm;
    ...
    public void ExplodeRandomWorm(InputAction.CallbackContext context)
    {
        //print("alll");
        if (context.ReadValue<float>() == 1.0f)
