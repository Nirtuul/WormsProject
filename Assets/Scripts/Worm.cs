...
public class Worm : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _target= transform.Find("Target/Sphere").gameObject;
        _missileForce = 200.0f;
                ...
                if(_isCurrentWorm)
                {
                    if (_thisWormsStuff.ReturnSelectedWeapon() == 0)
                    {
                        if (FireBazooka.interaction is SlowTapInteraction)
                        {
                            ...
                            _missileForce *= Mathf.Abs(_chargeTime)+1.0f;
    ...
    private void MissileInFlight()
    {
        if (_isCurrentWorm)
        {
            ...
            {
                _target = transform.Find("Target/Sphere").gameObject;
                Vector3 corrected = new Vector3(_target.transform.TransformDirection(_target.transform.position).z,
                    _target.transform.TransformDirection(_target.transform.position).y,
                    0.0f);
                _missileFlightVector = corrected - this.gameObject.transform.position;
                _missileFlightVector /= _missileFlightVector.magnitude;
            ...
            {
                _target = transform.Find("Target/Sphere").gameObject;
                Vector3 correctedRight = new Vector3(_target.transform.TransformDirection(_target.transform.position).z*-1,
                    _target.transform.TransformDirection(_target.transform.position).y,
                    0.0f);
                _missileFlightVector = correctedRight - this.gameObject.transform.position;
                _missileFlightVector /= _missileFlightVector.magnitude;
            ...
            if (!_isFacingLeft)
            {
                _missile.transform.position = new Vector3(_rb.transform.position.x + 0.5f, _rb.transform.position.y,
    ...
    private void MoveTarget()
    {
        if (_isCurrentWorm)
        {
            ...
            {
                _target.transform.RotateAround(this.gameObject.transform.position, Vector3.back, 1.0f);
            }
        }
